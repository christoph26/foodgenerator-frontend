...
function recipeStorageService($window, recipeService) {
    ...
    function getRecipesFromIdList(recipeIds, additionalInformation, additionalFieldName) {
        var recipes = [];
        for (var index in recipeIds) {
            //noinspection JSUnfilteredForInLoop
            (function (indexClosure) {
                recipeService.get(recipeIds[indexClosure]).then(function (response) {
                    var recipe = response.data;
                    if (additionalInformation && additionalFieldName) {
                        //noinspection JSUnfilteredForInLoop
                        recipe[additionalFieldName] = additionalInformation[indexClosure];
                    }
                    recipes.push(recipe);
                })
            })(index);
